...
const STEPS = [
  { icon: 'check-circle'  as const, title: 'Application received',  body: 'We have everything we need from you.',                                done: true  },
  { icon: 'search'        as const, title: 'Document review',       body: 'Our team checks your documents — usually within 2–3 business days.', done: false },
  { icon: 'bell'          as const, title: 'You get notified',      body: "We'll email you as soon as your account is approved.",               done: false },
  { icon: 'trending-up'   as const, title: 'Start earning!',        body: 'Go online and accept your first ride. Keep 100% of the fare.',       done: false },
];

const TIPS = [
  { icon: 'battery-charging' as const, text: 'Keep your phone charged when you go online' },
  { icon: 'map-pin'          as const, text: 'Learn the popular pickup spots in Siem Reap' },
  { icon: 'star'             as const, text: 'Friendly drivers get more repeat customers' },
  { icon: 'volume-2'         as const, text: 'Keep your phone on loud so you hear new requests' },
];
  ...
  return (
    <View style={s.root}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Pressable style={s.backBtn} onPress={async () => { await signOut(); router.replace('/login'); }}>
          <Text style={s.backBtnText}>← Sign out</Text>
        </Pressable>

        <View style={s.hero}>
          <View style={s.heroIconWrap}>
            <Feather name="send" size={36} color={JihColors.gold} strokeWidth={1.5} />
          </View>
              ...
              <View style={s.stepLeft}>
                <View style={[s.stepDot, step.done && s.stepDotDone]}>
                  <Feather
                    name={step.icon}
                    size={20}
                    color={step.done ? JihColors.gold : JihColors.muted}
                    strokeWidth={1.8}
                  />
                </View>
        ...
        <View style={s.tipsCard}>
          <View style={s.tipsTitleRow}>
            <Feather name="info" size={15} color={JihColors.white} />
            <Text style={s.tipsTitle}>While you wait</Text>
          </View>
          {TIPS.map(tip => (
            <View key={tip.text} style={s.tipRow}>
              <Feather name={tip.icon} size={13} color={JihColors.muted} />
              <Text style={s.tip}>{tip.text}</Text>
            </View>
          ))}
        </View>

        <Pressable style={s.contactBtn} onPress={() => Linking.openURL('mailto:support@jisworld.com').catch(() => {})}>
          <Feather name="mail" size={15} color={JihColors.white} />
          <Text style={s.contactBtnText}>Questions? Email us</Text>
        </Pressable>

        <Pressable style={s.signOutBtn} onPress={async () => { await signOut(); router.replace('/login'); }}>
...
const s = StyleSheet.create({
  ...
  heroIconWrap:    { width: 80, height: 80, borderRadius: 40, borderWidth: 1.5, borderColor: 'rgba(232,160,32,0.35)', alignItems: 'center', justifyContent: 'center', backgroundColor: JihColors.navyM },
  ...
  tipsTitleRow:    { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 2 },
  tipsTitle:       { color: JihColors.white, fontSize: 15, fontWeight: '700' },
  tipRow:          { flexDirection: 'row', alignItems: 'flex-start', gap: 10 },
  tip:             { color: JihColors.muted, fontSize: 13, lineHeight: 20, flex: 1 },

  contactBtn:      { backgroundColor: JihColors.navyM, borderRadius: 12, paddingVertical: 15, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, borderWidth: 1, borderColor: JihColors.navyXL },
